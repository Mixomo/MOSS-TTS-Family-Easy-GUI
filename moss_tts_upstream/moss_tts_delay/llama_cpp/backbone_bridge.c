...
/* Runtime bridge ABI used by the Easy GUI. */
int32_t bridge_api_version(void)
{
    return 2;
}

/* Load and apply one GGUF LoRA adapter dynamically.
 * A new adapter replaces the currently active adapter.
 * Returns 0 on success, negative values on load/apply failure. */
int32_t bridge_set_lora(bridge_handle_t *h, const char *lora_path, float scale)
{
    if (!h || !h->model || !h->ctx || !lora_path || !lora_path[0]) return -1;

    if (h->lora) {
        llama_set_adapters_lora(h->ctx, NULL, 0, NULL);
        llama_adapter_lora_free(h->lora);
        h->lora = NULL;
    }

    struct llama_adapter_lora *adapter = llama_adapter_lora_init(h->model, lora_path);
    if (!adapter) {
        fprintf(stderr, "bridge_set_lora: failed to load adapter from %s\n", lora_path);
        return -2;
    }

    struct llama_adapter_lora *adapters[1] = { adapter };
    float scales[1] = { scale };
    int32_t ret = llama_set_adapters_lora(h->ctx, adapters, 1, scales);
    if (ret != 0) {
        fprintf(stderr, "bridge_set_lora: llama_set_adapters_lora failed: %d\n", ret);
        llama_adapter_lora_free(adapter);
        return -3;
    }

    h->lora = adapter;
    fprintf(stderr, "bridge_set_lora: applied %s at scale %.4f\n", lora_path, scale);
    return 0;
}

int32_t bridge_clear_lora(bridge_handle_t *h)
{
    if (!h || !h->ctx) return -1;
    int32_t ret = llama_set_adapters_lora(h->ctx, NULL, 0, NULL);
    if (h->lora) {
        llama_adapter_lora_free(h->lora);
        h->lora = NULL;
    }
